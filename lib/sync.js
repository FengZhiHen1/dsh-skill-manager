...
export async function health({ root, state, apps, groups, skills }) {
  ...
  const issues = []
  // 先跑项目既有条目分类：项目目标的 local-*/wrong-target 由分类统一报告，
  // per-target 循环跳过已分类条目，避免同一冲突双重报告。
  ...
  for (const [pname, ppath] of Object.entries(state.projects)) {
    ...
    for (const e of entries) {
      if (e.kind === 'local-empty' || e.kind === 'local-skill' || e.kind === 'local-foreign' || e.kind === 'wrong-target') {
        projectKinds.set(`${pname}|${e.name}`, e.kind)
  ...
  for (const [name, targets] of desired) {
    for (const t of targets) {
      ...
      if (t.scope === 'project' && projectKinds.has(`${t.project}|${name}`)) {
        continue // 已由项目条目分类报告（local-*/wrong-target/managed-ok 均不重复）
      }
