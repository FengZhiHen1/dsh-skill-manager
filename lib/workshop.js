...
// - 配置校验：非空必须是绝对路径且目录存在，不满足拒绝保存（settings
//   register 的 validate 钩子）。
...
async function assertDirectory(dir) {
  try {
    ...
    throw new Error(`本地 skill 目录不是目录：${dir}`)
  } catch (error) {
    if (error instanceof Error && error.message.startsWith('本地 skill 目录')) throw error
    throw new Error(`本地 skill 目录不存在或不可访问：${dir}`)
  }
}

/**
 * 车间根门禁：返回当前配置的车间根，未配置时抛 workshop-unconfigured。
