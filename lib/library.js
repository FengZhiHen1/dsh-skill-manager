...
export async function scanLibrary(root, groups) {
  ...
  // 批量 git 指纹：单次 spawn 取全部路径（避免每 skill 一次进程）
  const fingerprints = await logHashes(root, items.map((it) => `${SKILLS_REL}/${it.dir}`))
  for (const it of items) {
    it.fingerprint = fingerprints.get(`${SKILLS_REL}/${it.dir}`) ?? null
  }
  ...
  try {
    ...
    for (const entry of disabled.filter((d) => d.isDirectory())) {
      ...
      items.push({
        ...
        dir: entry.name,
        description: '',
        origin: locked ? (locked.repo ? 'github' : 'local') : 'self',
        hasSkillMd: false,
        fingerprint: null,
        upstream: locked,
        missing: false,
