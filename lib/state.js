...
const CHECK_CACHE_REL = 'distributor/check-cache.json'

/**
 * 上游检查缓存（DSR-008 状态直显）：checkedAt + 按 skill 目录名的最近检查结果。
 * 只由 check/update/remove 维护；library 读取并随列表下发，页面打开不发网络请求。
 */
export async function loadCheckCache(root) {
  const data = await readJson(root, CHECK_CACHE_REL)
  if (data === null || typeof data !== 'object') return { checkedAt: null, results: {} }
  return {
...
export async function saveCheckCache(root, cache) {
  await writeJson(root, CHECK_CACHE_REL, cache)
}
