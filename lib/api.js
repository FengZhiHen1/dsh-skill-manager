 ...
 * globalRootPath 由 Host 经 dshHomePath 注入（mount-sync.md「DSH skill 根」）；
 * 缺省时 sync 层回退 ~/.dsh/skills（仅测试/纯函数路径可用缺省）。
 * @param {() => import('@deepseek-ai/dsh-settings').SettingsScope} scopeGetter
 */
function createSession(scopeGetter, listWorkspaces, getStore, backupsRoot, globalRootPath) {
...
export function buildApi(scopeGetter, { listWorkspaces = () => [], getStore, backupsRoot, globalRoot } = {}) {
  const session = () => createSession(scopeGetter, listWorkspaces, getStore, backupsRoot, globalRoot)
    ...
    async 'health'() {
      ...
      return {
        issues: await syncMod.health({
          ...
          skills: b.skills,
          workspaceIds: b.workspaceIds,
          globalRootPath: s.globalRootPath,
        }),
      }
    },
  }
