...
function createSession(scopeGetter) {
  ...
  return {
    root,
    async bundle() {
      const [state, appsDoc, groups, skills] = await Promise.all([
...
export function buildApi(scopeGetter) {
  ...
  return {
    async 'library'(payload) {
      ...
      const { desired } = syncMod.deriveDesired({ state: b.state, apps: b.apps, groups: b.groups, skills: b.skills })
    ...
    async 'groups/op'(payload) {
      ...
      if (action === 'create') groupsMod.createGroup(doc, String(payload.name ?? ''))
