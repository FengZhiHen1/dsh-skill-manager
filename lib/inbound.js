 ...
 */
export async function check({ root, store, names }) {
    ...
    try {
      ...
      if (!missing) {
        if (entry.content_hash == null) {
          ...
          modified = (await dirHash(dest)) !== entry.content_hash
        }
      }
 ...
 */
export async function update({ root, store, names, confirmLocalChanges = false, ctx }) {
  const entries = new Map(store.skillEntries())
  const targets = names && names.length > 0 ? names : [...entries.keys()]
  const localChanges = []
...
export async function restore({ root, store, id, backupsRoot, ctx }) {
  ...
  // 恢复 skills 记录（含原分组）；元数据缺失时按 self 重建。
  const record = meta.record && typeof meta.record === 'object'
    ? { ...meta.record }
    : {
        origin: 'self', repo: null, branch: null, commit: null,
        path_in_repo: null, content_hash: null, origin_path: null,
        installed_at: nowIso(), disabled: false, group: '默认',
      }
  record.disabled = false
  record.content_hash = record.content_hash ?? await dirHash(dest)
  const groupName = typeof record.group === 'string' && record.group !== '' ? record.group : '默认'
  if (groupName !== '默认' && !store.getGroup(groupName)) {
    await store.putGroup(groupName, { created_at: nowIso() })
  }
...
/** 禁用（R-13）：置 disabled 标记，目录原地不动，对账摘除其全部物化。 */
export async function disable({ root, store, name, ctx }) {
  const record = store.getSkill(name)
  if (!record) throw new SkillManagerError('not-found', `库中不存在 skill: ${name}`)
  if (!(await pathExists(safePath(root, name)))) throw new SkillManagerError('not-found', `目录不存在: ${name}`)
  if (record.disabled === true) return { name, sync: null }
  await store.putSkill(name, { ...record, disabled: true })
  const sync = await ctx.reconcile()
  return { name, sync }
}

/** 启用（R-13）：清 disabled 标记，对账按所属组重新物化。 */
export async function enable({ root, store, name, ctx }) {
  const record = store.getSkill(name)
  if (!record) throw new SkillManagerError('not-found', `库中不存在 skill: ${name}`)
  if (!(await pathExists(safePath(root, name)))) throw new SkillManagerError('not-found', `目录不存在: ${name}`)
  if (record.disabled !== true) return { name, sync: null }
  await store.putSkill(name, { ...record, disabled: false })
  const sync = await ctx.reconcile()
  return { name, sync }
}
