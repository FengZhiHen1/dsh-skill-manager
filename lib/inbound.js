...
function locateSkillDir(files, subdir) {
...
async function materializeSkillDir(payload, subdir) {
  const { files } = explodeZipball(payload)
  const dir = locateSkillDir(files, subdir)
...
export async function check({ root, names }) {
  ...
  for (const name of targets) {
    ...
    if (!entry) {
      out.push({ name, status: 'skipped', reason: '不在锁文件中' })
      continue
    }
    if (!entry.repo) {
      out.push({ name, status: 'skipped', reason: '本地导入，无上游可比' })
      continue
    }
  ...
  if (dirty) await saveLock(root, lock)
  return out
}

/** 更新（R-10；目录缺失时即使 commit 未变也拉回）。 */
export async function update({ root, names, ctx }) {
  const lock = await loadLock(root)
  const targets = names && names.length > 0 ? names : Object.keys(lock.skills)
  const results = []
  let changed = false
  for (const name of targets) {
    ...
    const { tmp, dir: skillDir } = await materializeSkillDir(payload, entry.path_in_repo ?? undefined)
