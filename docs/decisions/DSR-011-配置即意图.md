...
- `入站操作.md` 的 disable/enable 端点语义、`目录配置与状态存储.md` 的七表形状、`挂载与同步.md` 的 mount.add/remove 与 `mounts` 表被本决定改写，正文已按新状态重写。
- 禁用语义收窄：`disabled` 只是挂载意图（退出期望集），不再把条目排除出 `check`/`update` 的默认目标集。
- 出库不再清除 settings 意图；恢复后组归属以配置中残留的意图为准。
- 波及文档：`需求.md`（R-02/R-06/R-12/R-13/R-17/C-08/AC-13）、`目录配置与状态存储.md`、`入站操作.md`、`挂载与同步.md`、`插件运行时.md`。
