...
- 实现点：`core/model/intent.js` 的 `configSchema().groups` 默认值、`client/section.jsx` 的 `createGroup` 起步模板、`client/manage.jsx` 建组对话框提示语；回归闸 `test/dir.test.mjs`「配置 schema 默认与注册」。
