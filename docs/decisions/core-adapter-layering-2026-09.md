...
> 状态：已确认（2026-09-01，design-spec-workshop D2′，含用户修订「按域分目录」），代码实施待执行；实施前 `project-structure.md` 等文档按目标形态描述。
...
- `project-structure.md` 插件结构节整节重写（树、模块边界、依赖规则、全局约定）；本插件 README 模块布局节实施时重写。
