...
| `base/fsys.js` | 文件系统与路径原语收口：`safePath`、原子写、原子换装（同卷临时目录构建 + rename 交换）、`copyTree`（统一跳过 `.git`/`__pycache__`）、链接读写、路径归属判定（`withinRoot`/`canonicalPath`/`pathsEqual`） |
...
| `mount/derive.js` | 挂载推导（纯）：目标拓扑（`globalRoot`/`projectRootOf`/`targetDirOf`）、`deriveDesired`、`targetKey`（全库唯一）、工作区投影现算与失效引用警告 |
...
| `service.js` | 应用服务层：方法表与 payload 规约、`dispatch`（READ/NET/WRITE 队列策略，R-13）、每请求会话与 bundle 读模型组装、缓存策略、错误 → 传输中立 Result 与 repair facts 映射 |
