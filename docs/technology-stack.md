...
本文是 `dsh-skill-manager` 的技术栈定调文档，唯一拥有语言与运行时、依赖与构建方式、部署目标、兼容与安全约束及选型理由。项目结构边界与分层规则归 DSH_Plugins 仓库 AGENTS.md；各机制如何使用这些技术归 `technical-details/`。实质变更属于重大决策，须写入决策记录并核对依赖文档。
