...
- `piAgentDir`：pi agent 目录（DSR-019）；空串 = 自动探测 `<home>/.pi/agent`，探测不到即不接管 pi；非空必须是绝对路径（显式指定时目录不存在也按可用计，物化时按需创建）。
...
- `validate` 只做**形式校验**：`skillsDir`/`piAgentDir` 非空时必须是绝对路径；组名符合组名规则（见「名称与校验」）；挂载规则 `hosts` 非空；`skills` 意图形状合法（值为对象、`group` 为字符串）。
