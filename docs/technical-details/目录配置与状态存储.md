...
- `groups`：组集合，键为组名，值 `{ mounts: [{ scope, project, hosts }] }`；`scope` 取 `global | project`，`project` 为工作区 `workspaceId` 或 `null`（`global` 规则的 `project` 归一为 `null`）；`hosts ⊆ ["dsh","pi"]`，缺省 `["dsh"]`（存量无 `hosts` 键的规则天然仅 DSH，零迁移），空数组是死规则、写路径拦截。默认种子 = 「默认」组挂载全局（仅 DSH）。
