...
本文唯一拥有库内容的扫描、搜索、仓库探测、入库、检查、更新、本地导入、出库、备份恢复的操作语义、失败模式和后置条件，以及禁用/启用的行为语义。目录语义、settings 意图形状与状态域形状归 `目录配置与状态存储.md`，挂载对账归 `挂载与同步.md`，RPC 传输与调度归 `插件运行时.md`。以下操作均要求本地 skills 目录已配置；未配置时由传输层统一返回 `skilldir-unconfigured`（见 `插件运行时.md`），本文不重复该门禁。
