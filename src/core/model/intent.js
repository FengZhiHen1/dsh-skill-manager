...
/**
 * 无条件探测 pi agent 目录（PI_CODING_AGENT_DIR 环境变量优先，其次 <home>/.pi/agent）。
 * 探测的是事实不是意图：开关（pi 字段）只控制产不产期望；只要探测到，pi 根就在
 * 对账扫描范围内——关掉开关后 pi 侧自有链接按孤儿判据被摘除（干净退出语义）。
 * home/env 为测试注入点（生产缺省取进程值）。
 */
export function probePiAgentDir(home = homedir(), env = process.env) {
  const envDir = typeof env.PI_CODING_AGENT_DIR === 'string' && env.PI_CODING_AGENT_DIR !== '' ? env.PI_CODING_AGENT_DIR : null
  try {
    const candidate = envDir ?? join(home, '.pi', 'agent')
    return statSync(candidate).isDirectory() ? candidate : null
  } catch {
    return null // 探测失败 = pi 未安装/不可读，按未安装回落
  }
}
